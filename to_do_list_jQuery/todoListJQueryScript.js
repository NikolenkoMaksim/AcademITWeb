$(document).ready(function () {
    ...
    $("#add_button").click(function () {
        ...
        if (noteText === "") {
            errorMessageBlock.text("Пожалуйста, заполните поле ввода");
            return;
        }

        function setViewMode(listItem) {
            listItem.html("<span class='note_text'></span><button class='edit_button' type='button'>Редактировать</button><button class='delete_button' type='button'>Удалить</button>");
            ...
            listItem.find(".edit_button").click(function () {
                listItem.html("<div><input class='edit_text' type='text' maxlength='58'><button class='save_button' type='button'>Сохранить</button><button class='cancel_button' type='button'>Закрыть</button></div><div class='error_message_block'></div>");

                listItem.find(".edit_text").val(noteText);

                listItem.find(".save_button").click(function () {
                    if (listItem.find(".edit_text").val() === "") {
                        listItem.find(".error_message_block").text("Пожалуйста, заполните поле ввода");
                        return;
                    }
